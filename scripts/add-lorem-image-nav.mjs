// One-off: add "Placeholder Image Generator" to the Tools submenu, last
// position (D-89, renamed to /tools/lorem-image same session). Same pattern
// as the prior seven add-*-nav.mjs scripts — direct insert, bypasses the
// app's revalidateNav() on purpose (CLAUDE.md's active ISR-quota
...
const { error } = await supabase.from('nav_items').insert({
  ...
  label_bn: 'Placeholder Image জেনারেটর',
  url: '/tools/lorem-image',
  parent_id: TOOLS_PARENT_ID,
  sort_order: nextOrder,
})
if (error) throw error
console.log(`Inserted "Placeholder Image Generator" under Tools, sort_order ${nextOrder}.`)
