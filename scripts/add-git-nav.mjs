// One-off: adds a nav_items row for /git under the Docs mega-menu parent,
// same pattern as scripts/add-cybersecurity-nav.mjs. Direct insert, bypasses
// the app's revalidateNav() on purpose (CLAUDE.md's active ISR-quota
// constraint). Idempotent. Caller busts the local dev server's 'nav' cache
// tag afterward via POST /api/revalidate {tag:"nav"} against localhost only.
...
const item = { label: 'Git & GitHub', label_bn: 'Git ও GitHub', url: '/git' }
