// One-off: add "Number System Converter" to the Tools submenu, last
// position (D-93). Same pattern as the prior nine add-*-nav.mjs scripts —
...
const { error } = await supabase.from('nav_items').insert({
  ...
  label_bn: 'Number System কনভার্টার',
  url: '/tools/number-system',
  parent_id: TOOLS_PARENT_ID,
  sort_order: nextOrder,
})
if (error) throw error
console.log(`Inserted "Number System Converter" under Tools, sort_order ${nextOrder}.`)
