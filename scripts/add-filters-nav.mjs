// One-off: add "Filter Studio" to the Tools submenu, last position (D-82).
// Same pattern as scripts/add-units-nav.mjs / add-animation-nav.mjs — direct
...
const { error } = await supabase.from('nav_items').insert({
  label: 'Filter Studio',
  label_bn: 'Filter স্টুডিও',
  url: '/tools/filters',
  parent_id: TOOLS_PARENT_ID,
  sort_order: nextOrder,
})
if (error) throw error
console.log(`Inserted "Filter Studio" under Tools, sort_order ${nextOrder}.`)
