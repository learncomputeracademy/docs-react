// One-off: add "Animation Studio" to the Tools submenu, last position (D-80).
// Same pattern as scripts/add-units-nav.mjs — direct insert, bypasses the
// app's revalidateNav() on purpose (CLAUDE.md's active ISR-quota constraint).
// Caller is responsible for busting the local dev server's 'nav' cache tag
...
const { error } = await supabase.from('nav_items').insert({
  ...
  label_bn: 'Animation স্টুডিও',
  url: '/tools/animation',
  parent_id: TOOLS_PARENT_ID,
  sort_order: nextOrder,
})
if (error) throw error
console.log(`Inserted "Animation Studio" under Tools, sort_order ${nextOrder}.`)
